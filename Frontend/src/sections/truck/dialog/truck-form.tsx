import { Dialog, DialogActions, DialogContent, DialogTitle, Button, TextField, Select, MenuItem, SelectChangeEvent } from '@mui/material';
import { useState, useEffect } from 'react';
import { TruckFormState } from 'src/interfaces/truck';
...
export function TruckFormDialog({
    ...
    const [truck, setTruck] = useState<TruckFormState>({
      ...
      chassisCode: '',
      color: '',
      plantName: 0,  // Alterado para plantName para maior clareza
    });
    ...
    const [errors, setErrors] = useState<{ 
      ...
      model?: string;
      color?: string;
      plantName?: string;  // Altere aqui também
    }>({});
  
    useEffect(() => {
      const fetchDefinitions = async () => {
        try {
          const response = await fetch('http://localhost:7006/api/trucks/definitions');
    ...
    useEffect(() => {
      if (truckToEdit) {
        ...
        setTruck({ id: '', model: 0, manufacturingYear: 0, chassisCode: '', color: '', plantName: 0 });
      }
    }, [truckToEdit]);
    ...
    const validateManufacturingYear = (year: number) => {
      ...
      if (year < 1900) {
          setErrors((prev) => ({ ...prev, manufacturingYear: 'Por favor, insira um ano posterior a 1900' }));
          return false;
      }
      if ( year > currentYear) {
          setErrors((prev) => ({ ...prev, manufacturingYear: 'Por favor, insira um ano anterior ou igual ao atual.' }));
          return false;
      }
    ...
    const validateRequiredFields = () => {
      ...
      if (!truck.manufacturingYear) newErrors.manufacturingYear = 'O ano de fabricação é obrigatório.';
        ...
        <DialogContent>
          {definitions ? (
            <>
              ...
              {errors.plantName && <small style={{ color: 'red', marginLeft:13 }}>{errors.plantName}</small>}
            </>
          ) : (
